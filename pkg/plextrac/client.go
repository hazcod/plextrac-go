...
type Client struct {
	...
	// Lazily-initialized resource services.
	Flaws       *FlawService
...
func (c *Client) do(ctx context.Context, method, path string, body, out any) error {
	...
	for attempt := 0; attempt <= c.retries; attempt++ {
		...
		c.log.Debug("http", "method", method, "url", endpoint, "attempt", attempt)
		...
		lr := io.LimitReader(resp.Body, c.maxRespBytes+1)
		respBody, _ := io.ReadAll(lr)
		_ = resp.Body.Close()
		if int64(len(respBody)) > c.maxRespBytes {
			return fmt.Errorf("plextrac: response exceeds max size (%d bytes)", c.maxRespBytes)
		}
