...
import (
	...
	"github.com/cresco/plextrac-go/pkg/plextrac/mdhtml"
)
...
func Run(ctx context.Context, cfg Config) error {
	...
	if mode == "update" {
		...
		for i, f := range flaws {
			...
			if _, ok := index[key]; ok {
				...
				fmt.Fprintf(cfg.Out, "  SKIP %s: not found in report\n", findings[i].ID)
			}
		}
		flaws = filtered
		_ = filteredIDs
	...
	for i, r := range results {
		...
		fmt.Fprintf(cfg.Out, "  %-6s %s  (flaw_id=%s)\n", strings.ToUpper(r.Action), id, newID)
		if r.Action == "create" {
			okCreate++
		} else {
			okUpdate++
